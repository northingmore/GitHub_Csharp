// Задача 60. Сформируйте трёхмерный массив из неповторяющихся двузначных чисел. 
// Напишите программу, 
// которая будет построчно выводить массив, 
// добавляя индексы каждого элемента.
//  Массив размером 2 x 2 x 2
//  66(0,0,0) 25(0,1,0)
//  34(1,0,0) 41(1,1,0)
//  27(0,0,1) 90(0,1,1)
//  26(1,0,1) 55(1,1,1)



int SizeMasive()
{
    Console.WriteLine("Введи размер масива(строка, столбец, 3-я плоскость): ");
    return int.Parse(Console.ReadLine()!);
}

int[,,] CreationMasive()
{
    int ROWS = SizeMasive();
    int COLUMN = SizeMasive();
    int threePlane = SizeMasive();
    int[,,] three_DimensionalArray = new int[ROWS, COLUMN, threePlane];;
    return three_DimensionalArray;
}

int[,,] InitionalisationMasive(int[,,] masive)
{
    int count = 0;
    for (int i = 0; i < masive.GetLength(0); i++)
    {
        for (int j = 0; j < masive.GetLength(1); j++)
        {
            for (int k = 0; k < masive.GetLength(2); k++)
            {
                masive[k, i, j] += count;
                count += 3;
            }
        }
    }
    
    return masive;
}

void MasivePrint(int[,,] mas)
{
    for (int i = 0; i < mas.GetLength(0); i++)
    {
        for (int j = 0; j < mas.GetLength(1); j++)
        {
            Console.WriteLine();
            for (int k = 0; k < mas.GetLength(2); k++)
            {
                Console.Write($"{mas[i, j, k]}({i},{j},{k}) ");
            }
        }
    }
}

MasivePrint(InitionalisationMasive(CreationMasive()));